 ...
 * @see Request
 */
module.exports.getUserConnectionIDs = async function getUserConnectionIDs(sessionUser) {
...
module.exports.getAccountAges = async function getAccountAges(accounts) {
    ...
    if (accounts.has('twitter')) {
        ...
        twitterPromise = client.get('users/show', { user_id: accounts.get('twitter') })
    }
    ...
    if (accounts.has('reddit')) {
        ...
        redditPromise = await redditWrapper.getUser(accounts.get('reddit')).fetch()
    }
    ...
    if (twitterUser !== null) {
        accountsCreationDates.push({
            ...
            "creation_date": moment(twitterUser.created_at, 'dd MMM DD HH:mm:ss ZZ YYYY', 'en').toDate()
        })
    }
    if (twitchUser !== null) {
        accountsCreationDates.push({
    ...
    if (youtubeUser !== null) {
        accountsCreationDates.push({
            "type": "youtube",
            "id": accounts.get('youtube'),
            "creation_date": new Date(youtubeUser.data.items[0].snippet.publishedAt)
        })
    }
 ...
 * @see UserModel
 */
module.exports.verifyUser = async function verifyUser(accounts, serverID, user) {
